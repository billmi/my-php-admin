...
//            }
//        }
        $this->assign("_Faction",$this->request->action());
    ...
    final protected function editRow($model, $data, $where, $msg = false)
    {
        ...
        if(!empty($ids) || !empty($id)){
            ...
            $cModel = model($model)->create();
            $nId = $cModel->save($data);
            if (!$nId)
                $this->error("操作失败！");
            $this->success("操作成功！");
        }
    ...
    final public function getMenus()
    {
        ...
        if (is_administrator(UID)) {
            ...
            $where['type'] = \app\admin\model\Menu::USER_TYPE;
        }
...
//            }
        ...
        // 查找当前子菜单
        $pid = \think\Db::name('Menu')->where("pid !=0 AND url like '%{$controller}/" . "%'")->value('pid');
        if ($pid) {
            // 查找当前主菜单
